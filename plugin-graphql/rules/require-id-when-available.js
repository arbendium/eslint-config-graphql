...
import { getBaseType } from '../estree-converter/utils.js';
import {
	...
	create(context) {
		...
		function checkSelections(
			...
			function checkFields(rawType) {
				...
				// Don't provide suggestions for selections in fragments as fragment can be in a separate
				// file
				if ('type' in node) {
					problem.suggest = idNames.map(idName => ({
						...
						fix: fixer => {
							...
							insertNode = insertNode.kind === Kind.INLINE_FRAGMENT
